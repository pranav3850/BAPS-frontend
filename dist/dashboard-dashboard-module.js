...
var DashboardComponent = /** @class */ (function () {
    function DashboardComponent(dashboardService, apiService, router) {
    ...
    DashboardComponent.prototype.getOldDetails = function (data) {
        ...
        this.dashboardService.getOldDetails(data).subscribe(function (res) {
            debugger;
            _this.tot_mem = res[0].nooffamily - res.length;
    ...
    DashboardComponent.prototype.verifyandupdate = function () {
        var _this = this;
        this.timeLeft = 120;
        var data = {
        ...
        this.dashboardService.verifyUserOTP(data).subscribe(function (res) {
            if (res == 'wrong') {
                ...
                _this.dashboardService.updatePersonalInfo(_this.professionViewModel).subscribe(function (res) {
                    if (res != 'success') {
                        _this.apiService.showNotification('top', 'right', ' Error in Information Updation.', 'danger');
                    }
                    else {
                        if (res == 'success1') {
                            _this.apiService.showNotification('top', 'right', 'Please Login with updated number.', 'success');
                            localStorage.clear();
    ...
    DashboardComponent.prototype.resendUpdateOTP = function () {
        var _this = this;
        var data = {
    ...
    DashboardComponent.prototype.updateHarbhakatDetailsById = function () {
        ...
        if (this.Role == undefined) {
            ...
            this.dashboardService.updatePersonalInfo(this.professionViewModel).subscribe(function (res) {
                if (res != 'success') {
                    _this.apiService.showNotification('top', 'right', ' Error in Information Updation.', 'danger');
                }
                else {
        ...
        sweetalert2__WEBPACK_IMPORTED_MODULE_8___default.a.fire({
            ...
            if (result.value == true) {
                sweetalert2__WEBPACK_IMPORTED_MODULE_8___default.a.fire({
                    ...
                    if (result.value == true) {
                        _this.dashboardService.removeHaribhaktDetails(id).subscribe(function (res) {
                            if (res == 'success') {
                                var data = { mob: _this.mainMob };
                                _this.getOldDetails(data);
    ...
    DashboardComponent.prototype.resendOTP = function () {
        var _this = this;
        var data = {
    ...
    DashboardComponent.prototype.AddExistUser = function () {
        ...
        this.dashboardService.verifyUserOTP(data).subscribe(function (res) {
            if (res == 'wrong') {
                ...
                var test = {
                    familyId: _this.duplicateUser.familyId,
                    oldFamilyId: _this.familyId
                };
                _this.dashboardService.addFamilytoNew(test).subscribe(function (res) {
                    if (res.length > 0) {
                        _this.familyId = res[0].familyId;
                        var data_1 = { mob: _this.mainMob };
                        _this.professionModel = _this.duplicateFamily;
                        _this.professionModel.forEach(function (element) {
                            element.title = 'update relation';
                        });
                        _this.apiService.showNotification('top', 'right', 'Family added successfully.', 'success');
                        // this.getOldDetails(data);
                        _this.dashboardService.removeLastInsertedOTP(data_1).subscribe();
                    }
                });
            }
        });
    ...
    DashboardComponent.prototype.AddMoreMember = function () {
        ...
        this.professionModel.forEach(function (ele, ind) {
            debugger;
            ele.index = ind;
        });
        // this.professionModel.length = this.tot_mem;
        ...
        this.dashboardService.verifyNumber(val).subscribe(function (res) {
            if (res.length > 0) {
                res.forEach(function (element) {
                    if (element.contactNo == _this.professionModel[data].contactNo) {
                        _this.duplicateUser = element;
                    }
                });
